...
use Database\Seeders\AiForMilitaryCourseSeeder;
use Database\Seeders\FreeFlowDemoSeeder;

it('lists AI untuk Militer on the public catalog as a free Open Course', function () {
    $this->seed(FreeFlowDemoSeeder::class);
    $this->seed(AiForCampusCourseSeeder::class);
    $this->seed(AiForCorporateCourseSeeder::class);
    $this->seed(AiForMilitaryCourseSeeder::class);
    $this->seed(AgentAcademyCourseSeeder::class);

    $learner = User::query()->where('email', 'learner@enterlms.test')->first();
    $military = Course::query()->where('title', AiForMilitaryCourseSeeder::COURSE_TITLE)->first();

    expect($military)->not->toBeNull()
        ->and($military->visibility)->toBe('public')
        ->and($military->is_paid)->toBeFalse()
        ->and($military->isPublished())->toBeTrue()
        ->and($military->lessons()->count())->toBe(count(AiForMilitaryCourseSeeder::LESSON_TITLES));
    ...
    $this->actingAs($learner)
        ...
        ->assertSee(AiForMilitaryCourseSeeder::COURSE_TITLE)
...
it('lets a Learner self-enroll in AI untuk Militer', function () {
    $this->seed(FreeFlowDemoSeeder::class);
    $this->seed(AiForMilitaryCourseSeeder::class);

    $learner = User::query()->where('email', 'learner@enterlms.test')->first();
    $course = Course::query()->where('title', AiForMilitaryCourseSeeder::COURSE_TITLE)->first();
...
it('does not grant Restricted OpenClaw by finishing AI untuk Militer', function () {
    $this->seed(FreeFlowDemoSeeder::class);
    $this->seed(AiForMilitaryCourseSeeder::class);
    $this->seed(AgentAcademyCourseSeeder::class);
