...
EnterLMS is an AI-first LMS. A Learner takes a Course with a Tutor. An LMS Agent may operate the academy from outside. It is not a live-agent control plane. The public catalog may list many free Open Courses (ADR 014). “Not a generic AI school” no longer limits how many Open Courses that catalog holds.

The public catalog lists Open Courses only (free, self-enroll). Restricted Courses (Administrasi Agen OpenClaw) stay off that catalog. Completing an Open Course does not grant a Restricted Course. Learners on the Path Pengenalan → OpenClaw are granted Path enrollment by LMS Admin. OpenClaw stays locked until Pengenalan is complete. LMS Admin in this phase is only the founder.
